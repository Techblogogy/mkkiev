...
class PageNode(template.Node):
    ...
    def render(self, context):
        ...
        if self.starts_with:
            pages = pages.filter(url__startswith=self.starts_with.resolve(context, True))
...
def get_pages(parser, token):
    ...
    whose title is defined by the ``as`` clause.
    ...
    syntax_message = ("%(tag_name)s expects a syntax of %(tag_name)s ['url_starts_with'] [for user] as context_name" %
    ...
    if len(bits) % 2 == 0:
        prefix = parser.compile_filter(bits[1])
    else:
        prefix = None

    # The very last bit must be the context title
    if bits[-2] != 'as':
        raise template.TemplateSyntaxError(syntax_message)
    context_name = bits[-1]

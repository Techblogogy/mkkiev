...
class HTML2Text(HTMLParser.HTMLParser):
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if self.twttr:
            if start:
                self.out("<" + tag + "".join((' %s="%s"' % (k, v) if v else k for (k, v) in raw_attrs)) + ">")
            elif tag == "blockquote":
                self.twttr = False
            else:
                self.out("</" + tag + ">")
        ...
        if tag == "blockquote" and attrs.get("class", "") in ("twitter-tweet", "instagram-media"):
            if start:
                self.twttr = True
                self.out("\n")
                self.o("<" + tag + "".join((' %s="%s"' % (k, v) if v else k for (k, v) in raw_attrs)) + '"' + ">")
            else:
                self.o("</" + tag + ">")
        ...
        if tag == "blockquote" and not self.twttr:
            if start:
                self.p()
                self.o('> ', 0, 1)
                self.start = 1
        ...
        if tag == 'iframe' or tag == 'script':
            if start:
                attr_str = " ".join(['%s="%s"' % (attr, value) if value is not None else attr for attr, value in attrs.items()])
                self.o("<" + tag + (" " + attr_str).rstrip() + ">")
            else:
                self.o("</" + tag + ">")
            ...
            if self.a and ((self.p_p == 2 and self.links_each_paragraph) or force == "end"):
                ...
                for link in self.a:
                    if self.outcount > link['outcount']:
                        ...
                        if has_key(link, 'title') and link['title']: self.out(" (" + link['title'] + ")")
                        self.out("\n")
                    else:
                        newa.append(link)
    ...
    def entityref(self, c):
        if not self.unicode_snob and c in unifiable.keys() and not self.twttr:
            return unifiable[c]
        else:
            try:
                name2cp(c)
